...
HRESULT WasapiAudioClient::ActivateCompleted(IActivateAudioInterfaceAsyncOperation* operation) noexcept
{
    ...
    try {
        ...
        if (!m_isLowLatency) {
            LOGI() << "before m_audioClient->Initialize";
            check_hresult(m_audioClient->Initialize(AUDCLNT_SHAREMODE_SHARED,
                                                    ...
                                                    m_mixFormat.get(),
                                                    nullptr));
            LOGI() << "after m_audioClient->Initialize";
        } else {
            LOGI() << "before m_audioClient->InitializeSharedAudioStream";
            check_hresult(m_audioClient->InitializeSharedAudioStream(AUDCLNT_STREAMFLAGS_EVENTCALLBACK,
                                                                     m_minPeriodInFrames,
                                                                     m_mixFormat.get(),
                                                                     nullptr));
            LOGI() << "after m_audioClient->InitializeSharedAudioStream";
        }

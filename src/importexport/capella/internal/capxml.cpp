    ...
    while (e.readNextStartElement()) {
        ...
        if (tag1 == "drawObj") {
            if (e.attribute("base") == enharmonicNote) {
                while (e.readNextStartElement()) {
                    const QString& tag2(e.name().toString());
                    if (tag2 == "group") {
...
void Capella::capxLayoutStaves(XmlReader& e)
{
    int iStave = 0;
    while (e.readNextStartElement()) {
        if (e.name().toString() == "staffLayout") {

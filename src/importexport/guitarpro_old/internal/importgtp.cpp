...
void GuitarPro::initGuitarProDrumset()
{
    ...
    gpDrumset->drum(91)
        = DrumInstrument(QT_TRANSLATE_NOOP("drumset", "Snare (Rim shot)"), NoteHeadGroup::HEAD_DIAMOND, 3, DirectionV::UP);
...
{
    Text* text = Factory::createText(score->dummy());
...
void GuitarPro::createSlide(int sl, ChordRest* cr, int staffIdx, Note* /*note*/)
{
    ...
    if (sl == SHIFT_SLIDE || sl == LEGATO_SLIDE) {
        Glissando* s = Factory::createGlissando(cr);
        //s->setXmlText("");
    ...
    if (noteBits & BEAT_EFFECTS) {
        ...
        if (modMask1 & EFFECT_GRACE) {
            ...
            if (transition == 0) {
                ...
                Glissando* glis = Factory::createGlissando(score->dummy());
